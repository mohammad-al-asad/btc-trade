...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'transactions' | 'price'>('transactions');
  ...
  const fetchTransactions = async () => {
    try {
      const response = await fetch('/api/admin/transactions?status=PENDING&limit=50');
      if (response.ok) {
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      <div className="container mx-auto p-4 max-w-7xl">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-400">Manage transactions and system settings</p>
        </div>

        {/* Stats Cards */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-gray-800 rounded-2xl p-6">
            <h3 className="text-lg font-semibold text-gray-400">Pending Transactions</h3>
            <p className="text-3xl font-bold text-yellow-400">{stats.pending}</p>
          </div>
          <div className="bg-gray-800 rounded-2xl p-6">
            <h3 className="text-lg font-semibold text-gray-400">Pending Deposits</h3>
            <p className="text-3xl font-bold text-blue-400">{stats.pendingDeposits}</p>
          </div>
          <div className="bg-gray-800 rounded-2xl p-6">
            <h3 className="text-lg font-semibold text-gray-400">Pending Withdrawals</h3>
        ...
        {activeTab === 'price' && (
          <div className="space-y-6">
            ...
            {/* Recent Adjustments */}
            <div className="bg-gray-800 rounded-2xl p-6">
              <h2 className="text-xl font-semibold mb-4">Recent Price Adjustments</h2>
              {priceAdjustments.length > 0 ? (
                <div className="space-y-3">
                  {priceAdjustments.map((adjustment) => (
                    <div key={adjustment.id} className="flex items-center justify-between p-3 bg-gray-700 rounded-lg">
                      <div>
                      ...
                      <div className="text-right">
                        <p className={`text-sm font-semibold ${
                          adjustment.adjustment >= 0 ? 'text-green-400' : 'text-red-400'
                        }`}>
                ...
                <p className="text-gray-400 text-center py-4">No price adjustments yet</p>
              )}
            </div>
          </div>
        )}
